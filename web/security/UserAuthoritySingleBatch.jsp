...
<%@ include file="/includes/main.jsp"%>
		...
		function scopeChang(obj){
			...
			var processor = ($(obj).parent().next().children()[0]);
			if(obj.value == '2906'){
				$(processor).show();
			}
			else{
		 ...
		 function processResult(result){
		    	if (result.statusCode == DWZ.statusCode.ok)
		    		alertMsg.correct("<hi:text key="保存成功!" />");
		    	else
		    		alertMsg.error("<hi:text key="保存失败!" />");
		...
		function setUserInfo(json)
		{
			...
			$("input[extName='privilegeProcessor']").each(function (n){
				 $(this).val('');
				});
			...
			document.getElementById("user.creator.id").value = json.user.creator.id;
			 for (var i=0;i<json.userPrivileges.length;i++)
			 {
			    ...
			    if ( document.getElementById("selectOrg"+json.userPrivileges[i].authority.id)  != null )
		         ...
		         if(json.userPrivileges[i].privilegeProcessor)
		        	 document.getElementById("privilegeProcessor"+json.userPrivileges[i].authority.id).value=json.userPrivileges[i].privilegeProcessor;
  ...
  <div class="pageContent">
    <table width="20%" border="0" cellpadding="1"  align="left" >
								<tr>
									...
									<hi:displayMenu name="com.hi.tree.menu" menuName="userPrivilegeList" type="common" cssStyle="width:100%; height:100%; border: 0;float: auto;"/>
				...
				<table class="list nowrap" width="98%">
					<thead>
						<tr class="ListTableHeader" >  
							...
							<th><hi:text key="权限处理器" entity="UserAuthority" /></th>		
					...
					<c:forEach var="userAuthority" items="${userAuthorities}"  varStatus="userAuthorityIndex">
  						<tr name="mt_${userAuthority.authority.menuLink.menu.id}" class="mt_${userAuthority.authority.menuLink.menu.id}" id="mt_${userAuthority.authority.menuLink.menu.id}" style="display:none">
							...
							<td>
								<c:if test="${userAuthority.authority.authorityType==1}">					
									<input extName="privilegeProcessor" type="text" id="privilegeProcessor${userAuthority.authority.id}" name="userAuthorities[${userAuthorityIndex.count-1}].privilegeProcessor" value="${userAuthority.privilegeProcessor}" size="40" style="display:<c:choose><c:when test="${roleAuthority.scope == 2906}">block</c:when><c:otherwise>none</c:otherwise></c:choose>"/>
								</c:if>						
							</td>
							<td>
