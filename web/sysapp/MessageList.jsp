...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="messageList.action?lookup=${lookup}" onsubmit="return dwzSearch(this, '${targetType}');">
			...
			<tr>
				...
				<th width="90">
					<c:choose>
						...
						<c:otherwise><hi:text key="选择"/></c:otherwise>
					</c:choose>
				</th>
			</tr>
		</thead>				
		<tbody>
			...
			<tr>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btnSelect" href="javascript:$.bringBack({id:'${item.id}', receiverNames:'${item.receiverNames}',sender:'${item.sender}',createDate:'${item.createDate}',messageType:'${item.messageType}',sendDate:'${item.sendDate}',isSent:'${item.isSent}',description:'${item.description}'})" title="<hi:text key="选择消息"/>"><hi:text key="选择"/></a>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>
	...
	<div class="panelBar">
		<div class="pages">
			...
			<select name="pageInfo.pageSize" onchange="dwzPageBreak({targetType:'${targetType}', numPerPage:this.value})">
			 ...
			 <span><hi:text key="条"/>, <hi:text key="共"/>${pageInfo.totalPage}页 ${pageInfo.totalRecords}<hi:text key="条"/></span>
		</div>
