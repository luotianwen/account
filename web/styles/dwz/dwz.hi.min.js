...
v=v.replace(/\s+/g,"");
return this.optional(element)||v.match(/^(13[0-9]|15[0-9]|168|189)\d{8}$/);},"Please specify a valid mobile number");
...
options.changRemove=$this.attr("changRemove")||false;
$this.tabs(options);});
$("ul.tree",$p).jTree();
...
var op=$.extend({reverse:false,eventType:"click",changRemove:false,currentIndex:0,stTabHeader:"> .tabsHeader",stTab:">.tabsHeaderContent>ul",stTabPanel:"> .tabsContent",ajaxClass:"j-ajax",closeClass:"close",prevClass:"tabsLeft",nextClass:"tabsRight"},options);
return this.each(function(){
initTab($(this));});
function initTab(jT){
...
100);}else if("forward"==json.callbackType){
...
if(_lookup.currentGroup==$input.attr("lookupGroup")&&_lookup.currentName==$input.attr("lookupName")&&inputName.indexOf("pageInfo.")>=0){
if(index>0)
result+=",";
...
result+=":";}
if($input.attr("id")){
result+=document.getElementById($input.attr("id")).value;}else{
...
$suggest.css({
left:offset.left+'px',width:$width||'138px',
top:iTop+'px'}).show();
_lookup=$.extend(_lookup,{
...
break;
case 'attach':
html='<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'+'<input type="text" class="'+field.styleClass+'"  name="'+lookupFieldName+'" size="'+field.size+'" readonly="readonly"/>'+'<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+itemDetail+'" lookupName="'+field.lookupName+'" index="#index#" width="560" height="300" title="查找带回">查找带回</a>';
break;
case 'enum':
$.ajax({
...
break;
case 'date':
html='<input type="text" name="'+fieldName+'" class="date '+field.styleClass+'" pattern="'+field.patternDate+'" size="'+field.size+'"  readonly="readonly"/>'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
case 'datetime':
html='<input type="text" name="'+fieldName+'" class="date '+field.styleClass+'" pattern="'+field.patternDatetime+'" size="'+field.size+'"  readonly="readonly"/>'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
case 'hidden':
