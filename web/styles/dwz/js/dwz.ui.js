...
function initLayout(){
	...
	if(window.parent != window)
		$("#container .tabsPageContent").height($(window).height()).find("[layoutH]").layoutH()
	else
	...
	if(window.parent != window){
		var style = $("#container").attr("style") + "position:relative; top:0px; left:0px; ";
...
function initUI(_box){
	...
	$("div.tabs", $p).each(function(){
		...
		options.changRemove = $this.attr("changRemove") || false;  //j-hi zhanghao
		$this.tabs(options);
	});
	...
	if(!hi_helper){
		...
		$.ajax({
			url:"helper!refHelper.action",
			dataType:"json",
			async:false,
			cache: false,
			success: function(json){
				if(json.result){
					$("a.hi_helper").attr("title", json.title);
					$("a.hi_helper").show();
				}
				else
					$("a.hi_helper").hide();
