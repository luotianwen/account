...
var DWZ = {
	...
	jsonToString:function (o){
	    ...
	    if(typeof o == "object"){
	        if(!o.sort){
	            for(var i in o){
	            	var val = o[i];
	            	if(typeof val =="string" && val.indexOf("javascript") > 0) 
	            		continue;
	                r.push("\""+i+"\":"+DWZ.jsonToString(val));
	            }
	...
	ajaxError:function (xhr, ajaxOptions, thrownError){
		if(xhr.responseText.match("Session Timeout")){
				DWZ.loadLogin();
				return;
			}
		if (alertMsg) {
...
(function($){
	...
	$.extend(String.prototype, {
		...
		isUrl:function(){
			return (new RegExp(/^.+:\/\/([a-zA-Z0-9\-\.]+)([-\w .\/?%&=:]*)$/).test(this));
		},
