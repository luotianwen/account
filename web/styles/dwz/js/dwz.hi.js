	...
	$.fn.extend({
		removeSelected: function(options){
			...
			return this.each(function(){
				...
				$this.click(function(){
					var ids = getIds();
					if (!ids) {
						alertMsg.error($this.attr("msg") ||"请选择要删除的记录");
						return false;
					}
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				var $width=$input.attr('suggestwidth');
				function _show(){
					...
					$.ajax({
						...
						success: function(response){
							...
							$suggest.html('<ul style="width:100%" >'+html+'</ul>').find("li").hoverClass("selected").click(function(){
								_select($(this));
							});
			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(){
					...
					var field = {
						...
						defaultValue:$th.attr("defaultValue") || "",
						enumType:$th.attr("enumType") || "",
				...
				switch(field.type){
					...
					case 'enum':
						$.ajax({
							...
							data:{enumName:field.enumName, inputName:fieldName,type:field.enumType,defaultValue:field.defaultValue}, 
							success:function(response){
								html = response;
							}
						});
